...
//        // Replace the birthday into the year
        if(strpos($reqUrl, "birthday") !== false) {
            file_put_contents(dirname(__FILE__).'/output/komparu.log', "V3 URL BEFORE => $reqUrl\n", FILE_APPEND);
            $reqUrl = urldecode($reqUrl);
            file_put_contents(dirname(__FILE__).'/output/komparu.log', "V3 OLD URL => $reqUrl\n", FILE_APPEND);
            // First we grep YMD
            $day = array();
            preg_match('/\[birthday\]\[0\]\[0\]=([0-9]+)/', $reqUrl, $day);
            $month = array();
            preg_match('/\[birthday\]\[0\]\[1\]=([0-9]+)/', $reqUrl, $month);
            $year = array();
            preg_match('/\[birthday\]\[0\]\[2\]=([0-9]+)/', $reqUrl, $year);
            ...
            $reqUrl = str_replace('[birthday][0][0]='.$day[1],  '[birthday][0][0]=1', $reqUrl);
            $reqUrl = str_replace('[birthday][0][1]='.$month[1],'[birthday][0][1]=1', $reqUrl);
            $reqUrl = str_replace('[birthday][0][2]='.$year[1], '[birthday][0][2]='.$new->format('Y'), $reqUrl);
            file_put_contents(dirname(__FILE__).'/output/komparu.log', "V3 NEW URL => $reqUrl\n", FILE_APPEND);
            $reqUrl = urlencode($reqUrl);
            file_put_contents(dirname(__FILE__).'/output/komparu.log', "V3 URL AFTER => $reqUrl\n", FILE_APPEND);
        }
